---
title: Forex Forwards Explained
---

# What Are FX Forwards: How They Work, Pros & Cons, Markets

An FX forward is a private over-the-counter contract between two parties to exchange one currency for another at a fixed rate on a future date. The contract specifies the trade date, settlement date, notional amount, counterparties, and settlement method, and it can be structured in different forms such as outright forwards or non-deliverable forwards.

The main advantages of FX forwards are exchange rate certainty, protection against adverse currency movements, improved cash flow planning, contract customization, no upfront premium in most cases, and a simple hedging structure. The main risks are obligation risk, opportunity cost if rates move favorably, exposure to market volatility, and counterparty risk due to their over-the-counter nature.

FX forwards are mainly used by importers, exporters, multinational companies, banks, and institutional investors. Most participants use them to hedge currency exposure rather than to speculate on short-term price movements.
...
1. Trade date, when the rate is agreed
2. Settlement date, when currencies are exchanged
3. Forward rate, the fixed exchange rate agreed today
4. Notional amount, the contract size
5. Counterparties, the two parties to the agreement
6. Settlement method, either physical delivery or cash settlement
...
The 2 alternatives to FX forwards for retail traders are Forex CFDs and Forex futures. These FX forward alternatives provide access to currency trading through standardized, margin-based instruments that are designed for [active forex trading](what-is-forex-trading) rather than customized corporate hedging.

### What are forex CFDs?

Forex CFDs are contracts for difference that allow traders to [speculate on the price movements of currency pairs](forex) without owning the underlying currencies.
